...
// functions/get-account.ts
var get_account_exports = {};
__export(get_account_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(get_account_exports);
...
var handler = async (event) => {
  ...
  const isLocal = origin2.includes("localhost");
  ...
  try {
    ...
    const xummResponse = await axios_default.get(
    ...
    if (!account) {
      return {
        ...
        headers: {
          ...
          // Устанавливаем корректный CORS заголовок
          "Access-Control-Allow-Headers": "Content-Type"
        },
        body: JSON.stringify({ error: "Account not found in response" })
      };
    }
    const cookieSettings = isLocal ? "sessionId=abc123; Path=/; HttpOnly; SameSite=None" : "sessionId=abc123; Path=/; Secure; HttpOnly; SameSite=None";
    return {
      statusCode: 200,
      headers: {
        "Access-Control-Allow-Origin": origin2,
        // Поддержка CORS для корректного источника
        "Access-Control-Allow-Headers": "Content-Type",
